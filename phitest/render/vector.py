...
class Vector(Matrix):
	...
	def __iter__(self):
		return _VectorIterator(self, len(self))
	...
	def __getattr__(self, attr):
		...
		elif len(r)==4: return Vector4(self[r])#np.choose(r, self._value))
	#	else:
...
class GridShape(Vector):
	...
	def __init__(self, v):
		...
		if isinstance(v, (list, tuple, np.ndarray)):
			...
			assert v.shape==(3,) or v.shape==(4,) or v.shape==(5,), "Shape: %s"%(v.shape,)
	...
	def from_tensor(cls, tensor):
		if isinstance(tensor, (tf.Tensor, tf.Variable)):
			...
			raise TypeError("Creating GridShape from tensor with type '%s' is not supported.", type(tensor).__name__)
		return cls(shape)
...
"""
class TF_Vector(Vector):
	def __init__(self, *args, **kwargs):
		raise NotImplementedError

class TF_Vector3(TF_Vector):
	def __init__(self, v):
		if isinstance(v, (list, tuple, np.ndarray)):
			v = np.array(v, copy=True)
			assert v.shape==(3,)
			v = tf.constant(v.value)
		elif isinstance(v, (tf.Tensor, tf.Variable)):
			pass
		elif isinstance(v, TF_Vector3):
			v = v.value
		elif isinstance(v, Vector3):
			v = tf.constant(v.value)
		else:
