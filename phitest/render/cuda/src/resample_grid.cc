...
//	.Input("lookup: float32") //LuT if used
	...
	//.Attr("sampling_mode: {'LINEAR', 'LINEAR_MIP_LINEAR', 'TEX_LINEAR', 'NEAREST', 'TEX_NEAREST'} = 'LINEAR_MIP_LINEAR'")
	.Attr("interpolation: {'NEAREST', 'LINEAR', 'QUADRATIC'} = 'LINEAR'")
	.Attr("boundary: {'BORDER', 'CLAMP'} = 'BORDER'")
	.Attr("mipmapping: {'NONE', 'NEAREST', 'LINEAR'} = 'NONE'")
	.Attr("num_mipmaps: int = 0")
	.Attr("mip_bias: float = 0.0")
	//.Attr("inverse_transform: bool = false")
//	.Attr("use_texture: bool = false") // use texture memory for sampling, not compatible with all modes
...
//		c->set_output(1, c->input(6)); // TODO: if used
		return Status::OK();
	});
...
REGISTER_OP("LodTransform")
	...
	.Attr("linearize_depth: bool = true") // 
	.Attr("relative_coords: bool = false")
...
/*
void SampleRGridKernelLauncher(
...
void SampleRGGridKernelLauncher(
		const void* input, const long long int* input_shape,
		const float* M, const float* V, const float* P, const float* frustum, int32_t numCameras,
		uint8_t* mipAtlas,
		const Sampling::CoordinateMode coordinateMode,
		const Sampling::SamplerSettings, const bool globalSampling,
		void* output, const long long int* output_shape);
void SampleRGBAGridKernelLauncher(
		const void* input, const long long int* input_shape,
		const float* M, const float* V, const float* P, const float* frustum, int32_t numCameras,
		uint8_t* mipAtlas,
		const Sampling::CoordinateMode coordinateMode,
...
public:
	explicit SampleGridTransformOp(OpKernelConstruction *context) : OpKernel(context){
		...
		//OP_REQUIRES_OK(context, context->GetAttr("sampling_mode", &samplingMode));
		//OP_REQUIRES(context, setSamplingMode(m_samplingMode, samplingMode), errors::InvalidArgument("invalid sampling mode"));
		...
		if(s_interpolation.compare("QUADRATIC")==0) m_samplingSettings.filterMode = Sampling::SamplerSettings::FILTERMODE_QUADRATIC;
		else if(s_interpolation.compare("LINEAR")==0) m_samplingSettings.filterMode = Sampling::SamplerSettings::FILTERMODE_LINEAR;
		...
		//OP_REQUIRES_OK(context, context->GetAttr("use_texture", &m_samplingSettings.useTexture));
		...
		//OP_REQUIRES_OK(context, context->GetAttr("use_mipmaps", &m_mipmaps));
	...
	void Compute(OpKernelContext *context) override{
		...
		//const Tensor& tensor_lookup = context->input(5);
		...
		//const float* raw_lookup = nullptr;
		OP_REQUIRES(context, m_coordinateMode==Sampling::TransformLinDepth || m_coordinateMode==Sampling::TransformLinDepthReverse
			|| m_coordinateMode==Sampling::Transform || m_coordinateMode==Sampling::TransformReverse,
				errors::InvalidArgument("Invalid coordinate_mode"));
		//if(m_coordinateMode==Sampling::TransformLinDepth || m_coordinateMode==Sampling::TransformLinDepthReverse
		//	|| m_coordinateMode==Sampling::Transform || m_coordinateMode==Sampling::TransformReverse){
		...
		/*}else if(m_coordinateMode==Sampling::LuT){
			MYLOG("Check LuT");
			TensorShape lut_shape = tensor_lookup.shape();
			OP_REQUIRES(context, lut_shape.dims()==5 && lut_shape.dim_size(4)==4
				&& lut_shape.dim_size(1)==output_shape.dim_size(1)
				&& lut_shape.dim_size(2)==output_shape.dim_size(2)
				&& lut_shape.dim_size(3)==output_shape.dim_size(3),
				errors::InvalidArgument("Invalid lut shape (NDHWC):", lut_shape.DebugString(), " with C=4, DHW must macht output shape:", output_shape.DebugString()));
...
private:
	...
	//Sampling::SamplingMode m_samplingMode;
...
public:
	...
	/*	Tensor* lookup_grads = nullptr;
		float* raw_lookup_grads = nullptr;
		if(m_coordinateMode==Sampling::LuT){
			TensorShape lut_shape = tensor_lookup.shape();
			OP_REQUIRES_OK(context, context->allocate_output(1, lut_shape, &lookup_grads));
			raw_lookup_grads = lookup_grads->flat<float>().data();
		}else{
			TensorShape lut_shape;
			OP_REQUIRES_OK(context, context->allocate_output(1, lut_shape, &lookup_grads));
		}*/
...
public:
	explicit LoDTransformOp(OpKernelConstruction *context) : OpKernel(context){
		...
		bool linearizeDepth;
		OP_REQUIRES_OK(context, context->GetAttr("inverse_transform", &inverseTransform));
		OP_REQUIRES_OK(context, context->GetAttr("linearize_depth", &linearizeDepth));
		m_coordinateMode = linearizeDepth
			? (inverseTransform ? Sampling::TransformLinDepthReverse : Sampling::TransformLinDepth)
			: (inverseTransform ? Sampling::TransformReverse : Sampling::Transform);
	...
	void Compute(OpKernelContext *context) override{
		...
		ComputeLoDKernelLauncher(input_shape.dim_sizes().data(),
			...
			m_coordinateMode, //m_inverseTransform? Sampling::TransformLinDepthReverse : Sampling::TransformLinDepth,
			m_relativeCoords,
			output_grid->flat<float>().data(), output_shape.dim_sizes().data());
...
REGISTER_GPU(float);
#undef REGISTER_GPU
//REGISTER_KERNEL_BUILDER(Name("ResampleGridTransform").Device(DEVICE_CPU), ResampleGridTransformOp);
