...
/*
template<BlendMode BM>
struct BlendState<float1, BM>{
	__device__ static inline float1 blend(const float1 acc, const float1 cell_in){
		return acc + cell_in;
	}
	__device__ static inline float1 blendGradients(float1& grads_out, const float1 cell_in, float1& cell_out){
		return grads_out;
	}
};*/
...
/*
template<typename T>
struct BlendState<T, BLEND_ADDITIVE>{
	__device__ static inline T blend(const T acc, const T cell_in){
		return acc + cell_in;
	}
	__device__ static inline T blendGradients(T& grads_out, const T cell_in, T& cell_out){
		return grads_out;
	}
};*/
}
