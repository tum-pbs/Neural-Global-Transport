...
REGISTER_OP("RaymarchGridTransform")
	...
	//.Attr("coordinate_mode: {'TRANSFORM_LINDEPTH', 'TRANSFORM_LINDEPTH_REVERSE', 'TRANSFORM', 'TRANSFORM_REVERSE'} = 'TRANSFORM_LINDEPTH'")
	...
	.SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c)
	{
		...
		TF_RETURN_IF_ERROR(c->Subshape(c->input(0), 0, &batch));
		...
		TF_RETURN_IF_ERROR(c->Concatenate(batch, outShape, &outShape));
...
public:
	...
	void Compute(OpKernelContext *context) override{
		...
		OP_REQUIRES(context, output_shape.dim_size(0)==batch,
			errors::InvalidArgument("output_grads batch size does not match input batch size:", output_shape.dim_size(0), batch));
