...
struct CoordinateGenerator{
	...
	__device__ inline glm::vec4 getSampleCoord3D(glm::ivec3 globalIdx);
};

template<>
struct CoordinateGenerator<Transform>{
	bool inverse;
	bool linearDepth;
	bool useLoD;
	
	__device__ inline float _internal_calcLoD(glm::vec3 pos, glm::vec3 dx, glm::vec3 dy, glm::vec3 dz){
		const float px = glm::length(glm::vec3(dx.x, dy.x, dz.x));
		const float py = glm::length(glm::vec3(dx.y, dy.y, dz.y));
		const float pz = glm::length(glm::vec3(dx.z, dy.z, dz.z));
		return log2f(max(px, max(py, pz)));
	}
	
	__device__ inline glm::vec3 _internal_getPos3D(glm::ivec3 globalIdx){
		if(inverse){
			if(linearDepth) return OStoIDXlinearDepth(glm::vec4(globalIdx, 1.f), c_dimensions.input);
			else return OStoIDX(glm::vec4(globalIdx, 1.f), c_dimensions.input);
		}else{
			if(linearDepth) return IDXtoOSlinearDepth(globalIdx, c_dimensions.output_inv);
			else return IDXtoOS(globalIdx, c_dimensions.output_inv);
		}
			
	}
	
	__device__ inline glm::vec4 getSampleCoord3D(glm::ivec3 globalIdx){
		glm::vec4 pos = glm::vec4(_internal_getPos3D(globalIdx), 0.f);
		if(useLoD){
			const glm::vec3 d_x = glm::yxz(pos) - _internal_getPos3D(globalIdx+glm::ivec3(1,0,0));
			const glm::vec3 d_y = glm::yxz(pos) - _internal_getPos3D(globalIdx+glm::ivec3(0,1,0));
			const glm::vec3 d_z = glm::yxz(pos) - _internal_getPos3D(globalIdx+glm::ivec3(0,0,1));
			pos.w = _internal_calcLoD(pos, d_x, d_y, d_z);
		}
		return pos;
	}
};
...
template<>
struct CoordinateGenerator<LuT>{
	const float4* d_lut; //x,y,z,lod; device pointer
	bool relativePosition;
	
	__device__ inline glm::vec4 getSampleCoord3D(glm::ivec3 globalIdx){
		const float4 pos = vectorIO::readVectorType3D<float4, float4, glm::ivec3>(globalIdx, c_dimensions.output, d_lut);
		glm::vec4 samplingPos = glm::vec4(globalIdx, 0.f)*static_cast<float>(relativePosition) + vectorIO::toVector<float4, glm::vec4>(pos);
		return samplingPos;
	}
};
*/
...
template<>
__device__ inline glm::vec4 calcLoD<Transform>(const glm::vec3 idxCoords){
	const glm::vec3 d_x = (glm::xyz(IDXtoOS(idxCoords + glm::vec3(-1,0,0), c_dimensions.output_inv)) - \
							glm::xyz(IDXtoOS(idxCoords + glm::vec3(1,0,0), c_dimensions.output_inv))) *0.5f;
	const glm::vec3 d_y = (glm::xyz(IDXtoOS(idxCoords + glm::vec3(0,-1,0), c_dimensions.output_inv)) - \
							glm::xyz(IDXtoOS(idxCoords + glm::vec3(0,1,0), c_dimensions.output_inv))) *0.5f;
	const glm::vec3 d_z = (glm::xyz(IDXtoOS(idxCoords + glm::vec3(0,0,-1), c_dimensions.output_inv)) - \
							glm::xyz(IDXtoOS(idxCoords + glm::vec3(0,0,1), c_dimensions.output_inv))) *0.5f;
...
template<>
__device__ inline glm::vec4 calcLoD<TransformReverse>(const glm::vec3 idxCoords){
	const glm::vec3 d_x = (OStoIDX(glm::vec4(idxCoords + glm::vec3(-1,0,0), 1.f), c_dimensions.input) - \
							OStoIDX(glm::vec4(idxCoords + glm::vec3(1,0,0), 1.f), c_dimensions.input)) *0.5f;
	const glm::vec3 d_y = (OStoIDX(glm::vec4(idxCoords + glm::vec3(0,-1,0), 1.f), c_dimensions.input) - \
							OStoIDX(glm::vec4(idxCoords + glm::vec3(0,1,0), 1.f), c_dimensions.input)) *0.5f;
	const glm::vec3 d_z = (OStoIDX(glm::vec4(idxCoords + glm::vec3(0,0,-1), 1.f), c_dimensions.input) - \
							OStoIDX(glm::vec4(idxCoords + glm::vec3(0,0,1), 1.f), c_dimensions.input)) *0.5f;
...
__device__ inline float getQuadraticBSplineWeight(const float distance){
	if(distance<0.5f){
		return 0.75f - distance*distance;
	}else if(distance<1.5f){
		const float tmp = 1.5 - distance;
		return 0.5*tmp*tmp;
	}else{
		return 0.0f;
	}
...
/*
* Cell centers at +0.0 offset, spacing is uniformly 1.0.
* interpolate per-axis using quadratic B-splines
* https://cg.informatik.uni-freiburg.de/intern/seminar/animation%20-%20MPM%20survey%20-%202016.pdf page 33, equation 123
* weight =
*          3/4 - |x|^2 if 0<=x<1/2
*          1/2(3/2 - |x|)^2 if 1/2<=x<3/2
*          0 else
* where x is the distance between sampling location and interpolant
* this results in a 3x3x3 stencil
*/
template<typename T, SamplerSettings::BoundaryMode BM>
__device__ inline T read3DQuadratic(const glm::vec3 position, const T* buf, const glm::ivec3 buf_dims){
	...
	const glm::vec3 prevWeights = glm::vec3(getQuadraticBSplineWeight(prevDist.x), getQuadraticBSplineWeight(prevDist.y), getQuadraticBSplineWeight(prevDist.z));
	
	const glm::vec3 centerDist = glm::abs(position - glm::vec3(centerIdx));
	const glm::vec3 centerWeights = glm::vec3(getQuadraticBSplineWeight(centerDist.x), getQuadraticBSplineWeight(centerDist.y), getQuadraticBSplineWeight(centerDist.z));
	
	const glm::vec3 nextDist = glm::abs(position - glm::vec3(nextIdx));
	const glm::vec3 nextWeights = glm::vec3(getQuadraticBSplineWeight(nextDist.x), getQuadraticBSplineWeight(nextDist.y), getQuadraticBSplineWeight(nextDist.z));
	...
	if (BM == SamplerSettings::BORDER){// const 0 outside domain
		...
		if(BM == SamplerSettings::CLAMP){
			prevIdx = glm::clamp(prevIdx, glm::ivec3(0), buf_dims -1);
			centerIdx = glm::clamp(centerIdx, glm::ivec3(0), buf_dims -1);
			nextIdx = glm::clamp(nextIdx, glm::ivec3(0), buf_dims -1);
		}
		else if (BM == SamplerSettings::WRAP){//periodic
			prevIdx = vmath::positivemod<glm::ivec3, glm::ivec3>(prevIdx, buf_dims);
			centerIdx = vmath::positivemod<glm::ivec3, glm::ivec3>(centerIdx, buf_dims);
			nextIdx = vmath::positivemod<glm::ivec3, glm::ivec3>(nextIdx, buf_dims);
		}
		
		//read and interpolate along x
		const T v00 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, prevIdx.y, prevIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, prevIdx.y, prevIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, prevIdx.y, prevIdx.z, buf_dims, buf);
		const T v01 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, centerIdx.y, prevIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, centerIdx.y, prevIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, centerIdx.y, prevIdx.z, buf_dims, buf);
		const T v02 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, nextIdx.y, prevIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, nextIdx.y, prevIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, nextIdx.y, prevIdx.z, buf_dims, buf);
		...
		//read and interpolate along x
		const T v10 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, prevIdx.y, centerIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, prevIdx.y, centerIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, prevIdx.y, centerIdx.z, buf_dims, buf);
		const T v11 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, centerIdx.y, centerIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, centerIdx.y, centerIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, centerIdx.y, centerIdx.z, buf_dims, buf);
		const T v12 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, nextIdx.y, centerIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, nextIdx.y, centerIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, nextIdx.y, centerIdx.z, buf_dims, buf);
		...
		//read and interpolate along x
		const T v20 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, prevIdx.y, nextIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, prevIdx.y, nextIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, prevIdx.y, nextIdx.z, buf_dims, buf);
		const T v21 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, centerIdx.y, nextIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, centerIdx.y, nextIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, centerIdx.y, nextIdx.z, buf_dims, buf);
		const T v22 = 
			prevWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(prevIdx.x, nextIdx.y, nextIdx.z, buf_dims, buf) +
			centerWeights.x * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(centerIdx.x, nextIdx.y, nextIdx.z, buf_dims, buf) +
			nextWeights.x   * vectorIO::readVectorType3D<T, T, int32_t, glm::ivec3>(nextIdx.x, nextIdx.y, nextIdx.z, buf_dims, buf);
...
struct InterpolationData3D{
	...
	const T czcyfx;
	const T czcycx;
	const glm::vec3 ceilWeights;
}
*/
//#define ALIGN_CELL_CENTER
...
struct Sampler2{
	...
	__device__ inline T_DATA _internal_fetch3D(const glm::vec3 position, const glm::ivec3 buf_dimensions, const T_DATA *buf) const {
		...
		if(settings.filterMode==SamplerSettings::FILTERMODE_QUADRATIC){
			if(settings.boundaryMode==SamplerSettings::BORDER){
				data = read3DQuadratic<T_DATA, SamplerSettings::BORDER>(position - settings.cellCenterOffset, buf, buf_dimensions);
			}else if(settings.boundaryMode==SamplerSettings::CLAMP){
				data = read3DQuadratic<T_DATA, SamplerSettings::CLAMP>(position - settings.cellCenterOffset, buf, buf_dimensions);
			}
		}else if(settings.filterMode==SamplerSettings::FILTERMODE_LINEAR){
			if(settings.boundaryMode==SamplerSettings::BORDER){
	...
	__device__ inline T_DATA sample3D(const glm::vec4 position) const {
		/*if(settings.samplingMode==SamplerSettings::SAMPLINGMODE_TEXTURE){
			if(settings.mipMode!=SamplerSettings::MIPMODE_NONE){
				return tex3DLod<T_DATA>(texture, position.x+.5f, position.y+.5f, position.z+.5f, position.w);
			}else{
				return tex3D<T_DATA>(texture, position.x+.5f, position.y+.5f, position.z+.5f);
			}
		}else */
		if(settings.mipMode==SamplerSettings::MIPMODE_NONE){
...
__device__ inline T_DATA _internal_fetch3D(const SamplerSettings settings, const glm::vec3 position, const glm::ivec3 buf_dimensions, const T_DATA *buf){
	...
	// undo center offset to +0.0 as needed by the interpolation
	//position -= settings.cellCenterOffset;
	if(FM==SamplerSettings::FILTERMODE_QUADRATIC){
		data = read3DQuadratic<T_DATA, BM>(position - settings.cellCenterOffset, buf, buf_dimensions);
	}
	else if(FM==SamplerSettings::FILTERMODE_LINEAR){
