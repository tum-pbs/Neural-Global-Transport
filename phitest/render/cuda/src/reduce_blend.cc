...
//	.Input("z_dim: int32")
	...
	.SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c)
	{
		/*::tensorflow::shape_inference::ShapeHandle zDim;
		TF_RETURN_IF_ERROR(c->MakeShapeFromShapeTensor(1, &zDim));
		TF_RETURN_IF_ERROR(c->Subshape(zDim, 0, 1, &zDim));
		::tensorflow::shape_inference::ShapeHandle outShape;
		TF_RETURN_IF_ERROR(c->Merge(zDim, c->input(0), &outShape));
		*/
