...
__host__ inline void setFrustumParams(FrustumParams &frustum, const float* params){
	//memcpy(&frustum, params, sizeof(FrustumParams));
...
#ifdef CBUF_TRANSFORM_NORMAL
	mat4 M_model_inv_T;
#endif //CBUF_TRANSFORM_NORMAL
};
...
__host__ inline void setTransformations(Transformations& transforms, const float* M, const float* V, const float* P){
	...
	//transforms.M_modelView = transforms.M_model * transforms.M_view;
#ifdef CBUF_TRANSFORM_INVERSE
...
#ifdef CBUF_TRANSFORM_NORMAL
	#ifdef CBUF_TRANSFORM_INVERSE
	transforms.M_model_inv_T = vmath::transpose(transforms.M_model_inv);
	#else
	transforms.M_model_inv_T = vmath::transpose(vmath::inverse(transforms.M_model));
	#endif
#endif
...
__device__ inline float4 TransformVector(const mat4 m, const float4 v){
	return vmath::matmul(m, v);
}
__device__ inline float3 TransformPosition(const mat4 m, const float3 v){
	return make_float3(TransformVector(m, make_float4(v, 1.0f)));
}
__device__ inline float3 TransformDirection(const mat4 m, const float3 v){
	return make_float3(TransformVector(m, make_float4(v, 0.0f)));
}
